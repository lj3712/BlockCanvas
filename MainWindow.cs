...
namespace BlockCanvas {
    public sealed class MainWindow : Form {
        ...
        private void OpenLayout() {
            if (!ConfirmLoseChanges()) return;
            using var ofd = new OpenFileDialog { Filter = "BlockCanvas JSON (*.bcanvas.json)|*.bcanvas.json|JSON (*.json)|*.json|All files (*.*)|*.*" };
        ...
        private void SaveAsLayout() {
            using var sfd = new SaveFileDialog { Filter = "BlockCanvas JSON (*.bcanvas.json)|*.bcanvas.json|JSON (*.json)|*.json|All files (*.*)|*.*", FileName = Path.GetFileName(currentPath ?? "layout.bcanvas.json") };
